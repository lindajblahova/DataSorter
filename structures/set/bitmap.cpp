...
{
	//musim prepocitat index, kvoli posunu bazy (napr od 10)
...
bool structures::BitMap::isPresent(int index)
{
	...
	int cellIndex = realIndex / 64;
	int indexInCell = realIndex % 64;
	// zistujem hodnotu n-bitu, vracia uz booleam!
...
bool structures::BitMap::isSubSet(BitMap* other)
{
	...
	// podmnozina - ked obsahuje vsetky prvky vacsej
	for (int i = 0; i < array_->size(); i++)
	{
		// vysledkom bude and medzi mojim a other ull
...
{
	// this - other
	// rozdiel sa robi tak, ze vysledkom je A s vymazanymi prvkami prieniku s B
	BitMap* tmp = new BitMap(min_, max_);
	for (int i = 0; i < array_->size(); i++)
	{	// nemozem odcitat prvok ktory nie je v this
